        ...
        if(intent != null){
            ...
            switch (Objects.requireNonNull(intent.getAction())){
                case AppConstant.EXO_DOWNLOAD_ACTION_PAUSE:
                    ((Ooops) getApplication()).getDownloadManager().addDownload(downloadTracker.getDownloadRequest(uri), Download.STATE_STOPPED);

                    break;
                case AppConstant.EXO_DOWNLOAD_ACTION_START:
                    ((Ooops) getApplication()).getDownloadManager().addDownload(downloadTracker.getDownloadRequest(uri), Download.STOP_REASON_NONE);

                    break;

                case AppConstant.EXO_DOWNLOAD_ACTION_CANCEL:
                    ((Ooops) getApplication()).getDownloadManager().removeDownload(downloadTracker.getDownloadRequest(uri).id);
...
//                        .setGroup(GROUP_KEY_WORK_EMAIL)
                        .addAction(new NotificationCompat.Action(R.drawable.back_arrow, "Pause", pendingIntentPause))
                        .addAction(new NotificationCompat.Action(R.drawable.back_arrow, "Cancel", pendingIntentCancel))
                        .build();
